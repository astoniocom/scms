...
def index(request, sitemaps,
    ...
    for section, site in sitemaps.items():
        ...
        absolute_url = '%s://%s' % (protocol, req_site.domain)
        for page in range(1, site.paginator.num_pages + 1):
          sitemap_url = urlresolvers.reverse(sitemap_url_name, kwargs={'section': section, "page":page})
          sites.append('%s%s' % (absolute_url, sitemap_url))
...
def sitemap(request, sitemaps, section=None,
            template_name='sitemap.xml', content_type='application/xml', page=1):
